...
func (ss *session) newInteractiveIn() *interactiveIn {
	...
	if noColor := os.Getenv("NO_COLOR"); noColor == "" {
		...
		editor.Highlight = []readline.Highlight{
			{Pattern: newReservedWordPattern("HOST", "ALTER", "COMMIT", "CREATE", "DELETE", "DESC", "DROP", "EXIT", "HISTORY", "INSERT", "QUIT", "REM", "ROLLBACK", "SELECT", "SPOOL", "START", "TRUNCATE", "UPDATE", "AND", "FROM", "INTO", "OR", "WHERE", "TO", "TRANSACTION"), Sequence: "\x1B[36;49;1m"},
